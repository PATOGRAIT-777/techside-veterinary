...
export class CitasController {
  ...
  // --- NUEVOS ENDPOINTS ---
  @ApiOperation({ summary: 'Get initial booking data (branches/services)' })
  @Get('init-data')
  getInitData() {
    return this.citasService.getInitData();
  }

  @ApiOperation({ summary: 'Get user pets' })
  @Get('mis-mascotas/:usuarioId')
  getMisMascotas(@Param('usuarioId') usuarioId: string) {
    return this.citasService.getMascotasPorUsuario(usuarioId);
  }
...
getHorarios(@Param('medicoId') medicoId: string, @Param('fecha') fecha: string) {
  // Asegúrate de usar Query en lugar de Param si envías ?medicoId=...
  // Si usas @Query, cambia esto:
  ...
  cambiarEstado(
    @Param('id') id: string,
    @Body(new ZodValidationPipe(CambiarEstadoCitaDto)) dto: CambiarEstadoCitaDto,
    @CurrentUser() usuario: JwtPayload,
  ) {
